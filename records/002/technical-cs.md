...
Původní úplná lokální sada Build 002 prošla 74/74, bez přeskočených testů, v 578 980,073 ms. Původních 50 regresních testů zůstalo. Test 450 cyklů přeložil souhrnně 9 000 slov, překročil původních 8 192 slov i 256 položek, používal stále šest arén a skončil se 40 živými slovy ve dvou verzích; po odstranění nezůstala žádná živá verze. V tomto izolovaném testu s core + workspace měl rezidentní kód 7 078 slov a slovník 162 položek; během cyklů nerostly. Nejde o velikost celé městské aplikace. Tento zaznamenaný zátěžový běh trval 457 751,376 ms, nebyl izolovaným benchmarkem. Viz final-local-tests.txt.
...
Měření v [metrics.json](metrics.json) použilo Apple M5, 10 logických CPU, 32 GiB RAM, Darwin 25.6.0, Node 22.22.0, tři sekvenční nové BF starty v jednom procesu a generic Wasm executor. Mediány: boot 7 884,983 ms; uložení zdroje 110,113 ms; překlad/publikace 590,236 ms; první krok se třemi trasami 4 395,036 ms; průběžný krok 31,262 ms; příjezd 162,805 ms; teplý výpočet jedné trasy 1 154,756 ms; 20 cyklů reclaimingu 25 081,395 ms. Jednorázové načtení artefaktů trvalo 387,469 ms. Maximum RSS host procesu bylo 312 176 KiB, odděleně od 338 328 bajtů BF pásky. Prohlížeč navíc plánuje worker a vykresluje; tyto hodnoty nejsou slibem jeho latence nebo snímkové frekvence.
...
## Oprava nalezená při prvním veřejném ověření

První veřejný kandidát měl skutečně správný nativní stav, ale inspektor po importu připojil k nové trase verze 2 starou tabulku nákladů verze 1 z předchozího workeru. Toto vydání neprošlo a veřejný Site byl vrácen na ověřený Build 001; [receipt rollbacku](rollback-after-evidence-defect.json) i [chybný původní výstup inspektoru](public-new-decision-output.txt) zůstávají zachovány. Chyba byla v prezentaci důkazu, nikoli v BF výpočtu, ale i tak bránila dokončení buildu.

Oprava `routeEvidence` čte emitované záznamy v pořadí a připojí jen úplnou tabulku stejné verze. Změny silnic ji zneplatní a import začíná bez historie předchozího workeru. Host zde pouze přiřazuje záznamy; žádné skóre nevypočítává. Pokud snímek obsahuje nativní cache, ale nikoli její historické události, inspektor označí chybějící záznam. Samotná cache, bytecode a stav zůstávají nativní. Dva regresní testy kontrolují verzi, relaci, pořadí, neúplné tabulky a zneplatnění při změně dat.

Ve veřejném prohlížeči vznikl další nový zdroj: `dup 0= if drop 2 + else 9 * + then`. Skutečně stažený [public-inflight.8i](public-inflight.8i) v nové CLI instanci vrátil pro dobu 2 a mýto 5 skóre 47; viz [CLI receipt](public-image-cli.txt). Import obnovil krok 2, tři rozpracované úseky a verze 1/2. Krok 3 dokončil původní úseky v uzlech 8/4/1 a krok 4 zvolil ve verzi 2 trasu `8 4 5 6 7 11 15`, skóre 34. Opravený lokální inspektor tento výsledek znovu ověřil bez vypůjčené tabulky. Po odděleném uzavření Harbor Bridge už nová nativní tabulka obsahovala všech 46 řádků verze 2 a další trasa z uzlu 4 měla skóre 30; [důkaz](local-repaired-fresh-cost-inspector.txt).

Po opravě veřejného inspektoru prošlo nové úplné lokální ověření 76/76, bez selhání a přeskočení, v 542 348,846417 ms. [repair-full-local-tests.txt](repair-full-local-tests.txt) obsahuje celý běh včetně obou nových případů a opakování nativních regresí; nejde o izolované měření rychlosti.

## Identita ověřeného vydání

Koncový implementační commit je `94bb507bd14a9e91d7aeb440ad3f0d90056d31bb`. První běžný merge Build 002 je `b830e8733ac0656f796abf6614ac9056694328b7`; následný PR #2 obsahuje opravu a uzavření záznamu. Veřejně ověřená oprava byla vydána jako Site verze 3, zdrojová projekce `0ab3bc10a25da4cd0402284629e650955669dce3`, deployment `appgdep_6aa707b3fbc08191b6019ee11ffbeab0`. [publication.json](publication.json) obsahuje skutečné údaje a odkazy na všechny kontroly. Anonymní kontrola ověřila 71 souborů: 69 přesně, u dvou HTML pouze zachování původního zdroje s vloženým bezpečnostním skriptem hostingu. Hash nahraného gzip archivu se odlišuje od hostitelem normalizovaného tar archivu; oba jsou zapsané.

Metadata se uzavírají samostatným commitem běžného PR. Tag `build-002` a závěrečný [final-deployment.json](https://github.com/OnFiala/eight-instructions/releases/download/build-002/final-deployment.json) vážou skutečný finální merge, jeho CI a poslední projekci veřejných souborů. To brání kruhovému tvrzení, že commit předem zná vlastní hash nebo budoucí deployment. Veřejné chování opravy je doloženo zvlášť od posledního nasazení metadat.

Závěrečný reprodukční balíček skutečně stažený z opraveného veřejného webu prošel novou CLI instancí; viz [public-v3-reproduction-cli.txt](public-v3-reproduction-cli.txt). Nový export workspace má 451 513 bajtů a je bajtově shodný se stejnou dřívější posloupností vstupů. Po importu se opravovaný inspektor chová správně a poškozený import starý stroj nezničí. Vizuální kvalita prošla seberevizí s referencí; vrstvení dodávek záměrně upřednostňuje čitelnost jejich skutečné polohy před fyzikálně správným zakrytím. Safari, fyzická mobilní zařízení, plný screen-reader scénář a úplný WCAG audit nebyly provedeny.
