...
| Repository reproduces the result | Local `npm run verify` regenerated/checked kernel, compressed bytes, Wasm and raw source copies; all 50 tests passed. Clean public clone and Linux CI evidence are recorded in `verification.json` when completed. | Local PASS; final external reproduction tracked separately until observed. |
| Site claims match reality | Used real browser controls and both WebMCP tools; verified GCD, mutable routes, undo, image export/import and state reads. Invalid tool arguments fail before effects. The UI explicitly labels generated kernel, 16-bit cells, fixed capacities, host rendering and commit-versus-export. | PASS for locally tested behavior. Public deployment and asset identity remain a separate publication check. |
