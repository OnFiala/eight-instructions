...
| Failure / interruption | browser-invalid-edit.json preserves active version on compile refusal. browser-invalid-import-fixed.json visibly refuses malformed image and preserves the city. browser-queue-resume-fixed.json applies bridge input at the resumed boundary and avoids stale queued inputs after image replacement. |
| Actual export / import | browser-paused-export.json and browser-completed-export.json: actual downloaded machine files; fresh-worker continuation and completed round97 restored. Compressed fixtures are retained. Draft UI state is explicitly outside images. |
| Downloaded reproduction | reproduction-package-03.json, reproduction-local-fresh-cli-03.json and reproduction-gui-download-03.json: exact final ZIP delivered through the real UI; fresh extracted CLI matches61success inputs and13loop-rejection inputs. Public download replay remains a separate gate. |
| Visual / interaction | ../../design-qa.md and visual-comparison.json: actual1487×1058 target comparison,390×844mobile and820×1180tablet, accepted/rejected/repaired/completed states. Keyboard focus/Escape, internally scrollable mobile inspector and reduced motion verified. Invalid compositor captures are labelled and excluded from acceptance. |
...
Search1 checkpoint1 tests candidate5 for32logical rounds. Baseline17689 and
candidate17699 are native scores. A fresh validation checkpoint17 runs both
programs for16rounds with road0closed at4 and reopened at7: baseline17690,
candidate17698. Native acceptance occurs at25, copying only source. The real city
then pays three installed panels for factory7 and roads32/33 at37. Search2 and
search3 return equal scores and keep the usable baseline. Both three-panel goals
finish at97. These are recorded results, not a promise for arbitrary input.

## Limits

Finite grammar16, four automatic searches, fixed capacities, bounded history,
nonwrapping lifetime ceilings and one configured construction extension remain.
No optimality, endless growth, comprehensive accessibility, physical-device,
arbitrary-program safety proof or independent security audit is claimed. Browser
background suspension and tab closure stop automatic advancement. All original
failed attempts and superseded image receipts retain their actual identities.
