...
const kernel=await loadKernel(),source=await loadLibraries('autonomous-system.json');
...
if(check){
  const recorded=JSON.parse(await readFile('records/004/initial-image.json','utf8'));
...
await writeFile('records/004/initial-image.json',JSON.stringify(receipt,null,2)+'\n');
