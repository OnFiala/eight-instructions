...
const transcriptPath=process.argv[2],receiptPath=process.argv[3];
if(!transcriptPath||!receiptPath)throw new Error('Usage: node tools/check-browser-transcript.mjs CAPTURE.txt RECEIPT.json');
const raw=await readFile(transcriptPath,'utf8'),image=await readFile('dist/initial-industry.8i','utf8');
